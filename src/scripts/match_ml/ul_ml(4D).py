...
def match(injection_parameters, det, N_iter):
    ...
    # Generating the UNLENSED waveforms and Interpolating the PSD
...
plt.suptitle('UNLENSEDvsLENSED: Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Redshifted Lens Mass $M_{\\ell}^z$, and Impact Parameter $y$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_ml/ul_ml/UNLENSED(4D).png')
plt.close()

# Printing the required results

print('Injected - UNLENSED: \n %s'%(injection_parameters_0))
...
ht = inject_wf(injection_parameters_0)
...
ax.legend()
ax.grid(True)
plt.savefig('git_overlap/src/output/match_ml/ul_ml/UNLENSEDvsLENSED.png')
plt.show()
plt.close()