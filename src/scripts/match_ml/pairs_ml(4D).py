...
plt.suptitle('PAIRSvsLENSED: Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Redshifted Lens Mass $M_{\ell}^z$, and Impact Parameter $y$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_ml/pairs_ml/PAIRS(4D).png')
plt.close()
...
print('Recovered - PAIRS: Chirp Mass: %s, Sym. Ratio: %s, Mass1: %s, Mass2: %s; Redshifted Lens Mass: %s, Impact Parameter: %s; Match: %s'%(np.round(mchirpf_H1, 3), np.round(etaf_H1, 3), np.round(mass_1f_H1, 3), np.round(mass_2f_H1, 3), np.round(Ml_zf_H1, 3), np.round(yf_H1, 3), np.round(match_f_H1, 3)))
...
ax.set_xlim(injection_parameters_a['geocent_time']-0.5, injection_parameters_a['geocent_time']+0.3)
ax.legend()
ax.grid(True)
plt.savefig('git_overlap/src/output/match_ml/pairs_ml/PAIRSvsLENSED.png')
plt.show()
plt.close()
