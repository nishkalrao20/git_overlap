# 2 Dimensional (Chirp mass, Symmetric mass ratio) Nelder Mead maximization of Match on PAIRS waveforms
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter):
    ...
    for i in range(int(N_iter)):
        res = scipy.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}, tol=1e-3) 
...
plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$ and Sym Mass Ratio $\\eta$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match/PAIRS(2D).png')
plt.close()
