...
def match(injection_parameters_a, injection_parameters_b, det, N_iter):
    ...
    a_1_a, a_2_a = injection_parameters_a['a_1'], injection_parameters_a['a_2']
    a_1_b, a_2_b = injection_parameters_b['a_1'], injection_parameters_b['a_2']
    ...
    mchirp = (mchirp_a*waveform_metadata_a[det]['optimal_SNR']+mchirp_b*waveform_metadata_b[det]['optimal_SNR'])/(waveform_metadata_a[det]['optimal_SNR']+waveform_metadata_b[det]['optimal_SNR'])
    eta = (eta_a*waveform_metadata_a[det]['optimal_SNR']+eta_b*waveform_metadata_b[det]['optimal_SNR'])/(waveform_metadata_a[det]['optimal_SNR']+waveform_metadata_b[det]['optimal_SNR'])
    a_1, a_2 = (a_1_a*waveform_metadata_a[det]['optimal_SNR']+a_1_b*waveform_metadata_b[det]['optimal_SNR'])/(waveform_metadata_a[det]['optimal_SNR']+waveform_metadata_b[det]['optimal_SNR']), (a_2_a*waveform_metadata_a[det]['optimal_SNR']+a_2_b*waveform_metadata_b[det]['optimal_SNR'])/(waveform_metadata_a[det]['optimal_SNR']+waveform_metadata_b[det]['optimal_SNR'])
    ...
    a_1_0 = scipy.stats.uniform.rvs(size = int(1e4))*2*a_1
    a_2_0 = scipy.stats.uniform.rvs(size = int(1e4))*2*a_2
    ...
    a_1_arr = np.append(a_1, np.random.choice(a_1_0[idx], int(N_iter)-1))
    a_2_arr = np.append(a_2, np.random.choice(a_2_0[idx], int(N_iter)-1))
    ...
    for key in injection_parameters:
        injection_parameters[key] = (injection_parameters_a[key]*waveform_metadata_a[det]['optimal_SNR']+injection_parameters_b[key]*waveform_metadata_b[det]['optimal_SNR'])/(waveform_metadata_a[det]['optimal_SNR']+waveform_metadata_b[det]['optimal_SNR'])
    ...
    for i in range(int(N_iter)):
        res = scipy.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i], a_1_arr[i], a_2_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}) 
...
log_mismatch_H1, match_H1, params_H1 = match(waveform_metadata_a['H1']['parameters'], waveform_metadata_b['H1']['parameters'], 'H1', int(N_iter))
...
a_1_H1 = [[params_H1[i][j][2] for j in range(len(params_H1[i]))] for i in range(int(N_iter))]
a_2_H1 = [[params_H1[i][j][3] for j in range(len(params_H1[i]))] for i in range(int(N_iter))]
...
a_1_arr_H1 = [a_1_H1[i][len(eta_H1[i])-1] for i in range(N_iter)]
a_2_arr_H1 = [a_2_H1[i][len(eta_H1[i])-1] for i in range(N_iter)]
...
a_1f_H1, a_2f_H1 = a_1_arr_H1[min_idx], a_2_arr_H1[min_idx]
...
plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, and Dimensionless Spin Magnitudes $a_1, \ a_2$')

cmap = plt.get_cmap('plasma')
normalize = plt.Normalize(vmin=match_H1.min(), vmax=match_H1.max())
for i in range(N_iter):
    ax[0,0].plot(mchirp_H1[i], c=cmap(normalize(match_H1[i])))
    ax[0,1].plot(eta_H1[i], c=cmap(normalize(match_H1[i])))
...
ax[0,1].set_title('Sym Mass Ratio $\\eta$') 
ax[0,1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[0,1].axhline(y=eta_b, color='r', linestyle='--', label='$\\eta_B$')

mc12_c = ax[0,2].scatter(mchirp_arr_H1, eta_arr_H1, c = match_H1, cmap='plasma')
ax[0,2].scatter(mchirp_a, eta_a, facecolors = 'None', edgecolors='black')
ax[0,2].scatter(mchirp_b, eta_b, facecolors = 'None', edgecolors='black')
ax[0,2].scatter(mchirpf_H1, etaf_H1, facecolors = 'None', edgecolors='black')
ax[0,2].text(mchirp_a, eta_a,'A', verticalalignment='top', horizontalalignment='left')
ax[0,2].text(mchirp_b, eta_b,'B', verticalalignment='top', horizontalalignment='left')
ax[0,2].text(mchirpf_H1, etaf_H1,'F', verticalalignment='bottom', horizontalalignment='right')
ax[0,2].set_xlabel('$\mathcal{M}$')
ax[0,2].set_ylabel('$\eta$')
ax[0,2].set_title('Mass Ratio $\\eta$ $-$ Chirp Mass $\\mathcal{M}$')
clb1 = plt.colorbar(mc12_c, ax=ax[0,2])
clb1.ax.set_title('$Match$')

normalize = plt.Normalize(vmin=match_H1.min(), vmax=match_H1.max())
for i in range(N_iter):
    ax[1,0].plot(a_1_H1[i], c=cmap(normalize(match_H1[i])))
    ax[1,1].plot(a_2_H1[i], c=cmap(normalize(match_H1[i])))
ax[1,0].set_title('Dimensionless Spin magnitude: $a_1$')
ax[1,0].axhline(y=a_1_a, color='r', linestyle='--', label='$a_1_A$')
ax[1,0].axhline(y=a_2_a, color='r', linestyle='--', label='$a_2_B$')
ax[1,1].set_title('Dimensionless Spin magnitude: $a_2$')
ax[1,1].axhline(y=a_2_a, color='r', linestyle='--', label='$a_2_A$')
ax[1,1].axhline(y=a_2_b, color='r', linestyle='--', label='$a_2_B$')

s12_c = ax[1,2].scatter(a_1_arr_H1, a_2_arr_H1, c = match_H1, cmap='plasma')
ax[1,2].scatter(a_1_a, a_2_a, facecolors = 'None', edgecolors='black')
ax[1,2].scatter(a_1_b, a_2_b, facecolors = 'None', edgecolors='black')
ax[1,2].scatter(a_1f_H1, a_2f_H1, facecolors = 'None', edgecolors='black')
ax[1,2].text(a_1_a, a_2_a,'A', verticalalignment='top', horizontalalignment='left')
ax[1,2].text(a_1_b, a_2_b,'B', verticalalignment='top', horizontalalignment='left')
ax[1,2].text(a_1f_H1, a_2f_H1,'F', verticalalignment='bottom', horizontalalignment='right')
ax[1,2].set_xlabel('$a_1$')
ax[1,2].set_ylabel('$a_2$')
ax[1,2].set_title('Dimensionless Spin magnitudes: $a_1-a_2$')
clb2 = plt.colorbar(s12_c, ax=ax[1,2])
clb2.ax.set_title('$Match$')

fig.tight_layout()
plt.savefig('git_overlap/src/output/match/PAIRS(4D).png')
plt.close()
