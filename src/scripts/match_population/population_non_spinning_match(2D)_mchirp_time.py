...
for k in range(N_sampl):
    ...
    waveform_metadata_a.append(pickle.load(open('git_overlap/src/output/match_population/match_population_non_spinning/injections/GW Waveform A Meta Data %s.pkl' % indices[k], 'rb')))   # Importing Waveform Meta Data
    for key, val in waveform_metadata_a[k]['H1']['parameters'].items():   # Setting the variables
        if key in data_a:
            data_a[key][k] = val

    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_population/match_population_non_spinning/injections/GW Waveform B Meta Data %s.pkl' % indices[k], 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    np.savetxt('git_overlap/src/output/match_population/match_population_non_spinning/outputs/PAIRS(2D) %s.csv'%(k+1), np.column_stack((match_f_H1, mchirpf_H1, etaf_H1, mchirp_b[k], mchirp_a[k], snr_b[k], snr_a[k], delta_tc[k])), delimiter=',')

np.savetxt('git_overlap/src/output/match_population/match_population_non_spinning/outputs/output.csv', np.column_stack((match_f_snr, mchirpf_snr, etaf_snr, mchirp_b, mchirp_a, snr_b, snr_a, delta_tc)), delimiter=',')