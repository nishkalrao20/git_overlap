...
for k in range(N_sampl):
    ...
    waveform_metadata_a.append(pickle.load(open('git_overlap/src/output/match_population/run_match_population_equal_spins/injections/GW Waveform A Meta Data %s.pkl' % indices[k], 'rb')))   # Importing Waveform Meta Data
    for key, val in waveform_metadata_a[k]['H1']['parameters'].items():   # Setting the variables
        if key in data_a:
            data_a[key][k] = val

    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_population/run_match_population_equal_spins/injections/GW Waveform B Meta Data %s.pkl' % indices[k], 'rb')))   # Importing Waveform Meta Data
...
print("Running on processor num:", rank)

for batch_index in range(N_sampl // batch_size):

    for ii in range(batch_index * batch_size, (batch_index + 1) * batch_size):

        log_mismatch_H1, match_H1, params_H1 = match(waveform_metadata_a[ii]['H1']['parameters'], waveform_metadata_b[ii]['H1']['parameters'], 'H1', int(N_iter), ii)
        ...
        mchirpf_snr[ii], etaf_snr[ii], eff_spinf_snr[ii] = mchirpf_H1, etaf_H1, eff_spinf_H1
        np.savetxt('git_overlap/src/output/match_population/run_match_population_equal_spins/outputs/PAIRS(3D) %s.csv'%(ii+1), np.column_stack((match_f_H1, mchirpf_H1, etaf_H1, eff_spinf_H1, mchirp_b[ii], mchirp_a[ii], eta_a[ii], eta_b[ii], eff_spin_a[ii], eff_spin_b[ii], snr_b[ii], snr_a[ii], delta_tc[ii])), delimiter=',')
