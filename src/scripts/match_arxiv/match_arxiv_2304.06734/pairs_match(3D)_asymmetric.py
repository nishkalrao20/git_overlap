...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_arxiv/match_equal/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_arxiv/match_equal/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_arxiv/match_asymmetric/outputs/PAIRS(3D)ASYMMETRIC %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
plt.suptitle('ASYMMETRIC: Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Luminosity distance $d_L$, vs $\\Delta t_c$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_arxiv/match_asymmetric/PAIRS(3D)ASYMMETRIC.png')
plt.close()