...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_arxiv/match_asymmetric2/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_arxiv/match_asymmetric2/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_arxiv/match_asymmetric2/outputs/PAIRS(3D)ASYMMETRIC2 %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
plt.suptitle('ASYMMETRIC2: Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Luminosity distance $d_L$, vs $\\Delta t_c$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_arxiv/match_asymmetric2/PAIRS(3D)ASYMMETRIC2.png')
plt.close()