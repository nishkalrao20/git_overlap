# 2 Dimensional (Chirp mass, Symmetric mass ratio) Nelder Mead maximization of Match on PAIRS waveforms, by varying SNR of SINGLES_B relative to SINGLES_A
...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_arxiv/match_snr/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
luminosity_distance_a, snr_a = waveform_metadata_a['H1']['parameters']['luminosity_distance'], np.abs(waveform_metadata_a['H1']['optimal_SNR'])

N_sampl = 5
waveform_metadata_b = []
mass_1_b, mass_2_b, q_b, mchirp_b, eta_b, luminosity_distance_b, snr_b = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)

for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_arxiv/match_snr/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$ and Sym Mass Ratio $\\eta$ with SNR Ratio %s'%np.divide(snr_b[k],snr_a))
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_arxiv/match_snr/outputs/PAIRS(2D)SNR %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,2, figsize=(12, 6))
mc12_c = ax[0].scatter(np.divide(snr_b,snr_a), mchirpf_snr, c = match_f_snr, cmap='plasma')
ax[0].axhline(y=mchirp_a, color='r', linestyle='--', label='$\\mathcal{M}_A$')
ax[0].axhline(y=mchirp_b[0], color='r', linestyle='--', label='$\\mathcal{M}_B$')
ax[0].set_ylabel('$\mathcal{M}$')
ax[0].set_xlabel('$\\mathrm{SNR_B/SNR_A}$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs SNR Ratio')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(np.divide(snr_b,snr_a), etaf_snr, c = match_f_snr, cmap='plasma')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$\\mathrm{SNR_B/SNR_A}$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs SNR Ratio')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_arxiv/match_snr/PAIRS(2D)SNR.png')
plt.close()