...
injection_parameters_a = {
    "chirp_mass": 28.1, # Chirp mass of the binary black holes in solar masses
    "mass_ratio": 0.806, # Mass ratio of the binary
...
N_sampl = 5
luminosity_distance = [injection_a['luminosity_distance'][nmap_a], injection_a['luminosity_distance'][nmap_a]*1.5, injection_a['luminosity_distance'][nmap_a]*2, injection_a['luminosity_distance'][nmap_a]*2.5, injection_a['luminosity_distance'][nmap_a]*3]
injection_parameters_b = []
for i in range(N_sampl):
    injection_parameters_b.append({
        "chirp_mass": 26.6, # Chirp mass of the binary black holes in solar masses
        "mass_ratio": 0.806, # Mass ratio of the binary
        "luminosity_distance": luminosity_distance[i], # Distance in Megaparsecs
        "a_1": injection_a['a_1'][nmap_a], # Dimensionless spin of the heavier black hole
        "a_2": injection_a['a_2'][nmap_a], # Dimensionless spin of the lighter black hole        "ra": injection_a['ra'][nmap_a], # Right ascension of the source, in degrees
        "ra": injection_a['ra'][nmap_a], # Right Asencion of the source, in radians
        "dec": injection_a['dec'][nmap_a], # Declination of the source, in radians
        ...
        "psi": injection_a['psi'][nmap_a], # Polarization angle
        "incl": injection_a['iota'][nmap_a], # Inclination of the orbital angular momentum with respect to the line of sight
        "geocent_time": injection_a['geocent_time'][nmap_a]+0.025 # GPS time of the event
    })
...
with open('git_overlap/src/output/match_arxiv/match_snr/injections/GW Waveform A Meta Data.pkl', 'wb') as file:
...
for i in range(N_sampl):
    ...
    with open('git_overlap/src/output/match_arxiv/match_snr/injections/GW Waveform B Meta Data %s.pkl'%(i+1), 'wb') as file:
