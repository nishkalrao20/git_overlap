...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_arxiv/match_time_phase/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
N_sampl = 8
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, q_b, luminosity_distance_b, phase_b, delta_tc = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_arxiv/match_time_phase/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
    ...
    luminosity_distance_b[k], phase_b[k] = waveform_metadata_b[k]['H1']['parameters']['luminosity_distance'], waveform_metadata_b[k]['H1']['parameters']['phase']
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Luminosity distance $d_L$, with $\\Delta t_c=$ %s, Phase Diff. $\\Delta \\phi=$ %s'%(delta_tc[k], phase_b[k]))
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_arxiv/match_time_phase/outputs/PAIRS(3D)TIME_PHASE %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
mc12_c = ax[0].scatter(range(len(phase_b)), mchirpf_snr, c = match_f_snr, cmap='plasma')
ax[0].set_xticks(range(len(phase_b)), ['({}, {})'.format(a, b) for a, b in zip(phase_b, np.round(delta_tc,2))], rotation=45, ha='right')
...
ax[0].set_xlabel('$(\\Delta\\phi, \ \\Delta t_c)$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs $(\\Delta\\phi, \ \\Delta t_c)$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(range(len(phase_b)), etaf_snr, c = match_f_snr, cmap='plasma')
ax[1].set_xticks(range(len(phase_b)), ['({}, {})'.format(a, b) for a, b in zip(phase_b, np.round(delta_tc,2))], rotation=45, ha='right')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$(\\Delta\\phi, \ \\Delta t_c)$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs $(\\Delta\\phi, \ \\Delta t_c)$')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
dL12_c = ax[2].scatter(range(len(phase_b)), luminosity_distancef_snr, c = match_f_snr, cmap='plasma')
ax[2].set_xticks(range(len(phase_b)), ['({}, {})'.format(a, b) for a, b in zip(phase_b, np.round(delta_tc,2))], rotation=45, ha='right')
...
ax[2].set_ylabel('$d_L$')
ax[2].set_xlabel('$(\\Delta\\phi, \ \\Delta t_c)$')
ax[2].set_title('Recovered Luminosity Distance $d_L$ vs $(\\Delta\\phi, \ \\Delta t_c)$')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_arxiv/match_time_phase/PAIRS(3D)TIME_PHASE.png')
plt.close()