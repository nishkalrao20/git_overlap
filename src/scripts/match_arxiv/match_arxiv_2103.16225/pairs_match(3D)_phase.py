...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_arxiv/match_phase/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
phase_a = waveform_metadata_a['H1']['parameters']['phase']

N_sampl = 10
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, q_b, phase_b = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_arxiv/match_phase/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_arxiv/match_phase/outputs/PAIRS(3D)PHASE %s.png'%(k+1))
    plt.close()
...
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs Phase $\\phi_B$')
...
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$\\phi_B$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs Phase $\\phi_B$')
...
ax[2].set_ylabel('$\\phi$')
ax[2].set_xlabel('$\\phi_B$')
ax[2].set_title('Recovered Phase $\\phi$ vs Phase $\\phi_B$')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_arxiv/match_phase/PAIRS(3D)PHASE.png')
plt.close()