...
for i in tqdm(range(len(combinations))): 
    ...
    outdir = dir+'/pe/PE_{}_{}_{}/'.format(combinations[i][0], combinations[i][1], combinations[i][2])
    ...
    ini_file = '/home/nishkal.rao/git_overlap/src/scripts/pe_population_3G/config.ini'
    call("cp %s %s"%(ini_file, outdir), shell=1)
    os.chdir(outdir)

    with open('config.ini', 'r') as base_file:
    ...
    call('bilby_pipe config.ini --submit', shell=1)