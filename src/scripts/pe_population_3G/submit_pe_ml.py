...
for i in tqdm(range(len(combinations))): 
    ...
    outdir = dir+'/pe/PE_ML_{}_{}_{}/'.format(combinations[i][0], combinations[i][1], combinations[i][2])
    ...
    ini_file = '/home/nishkal.rao/git_overlap/src/scripts/pe_population_3G/config_ml.ini'
    call("cp %s %s"%(ini_file, outdir), shell=1)
    os.chdir(outdir)

    with open('config_ml.ini', 'r') as base_file:
    ...
    with open('config_ml.ini', 'w') as ini_file: 
        ini_file.write(ini_content)

    call('bilby_pipe config_ml.ini --submit', shell=1)