...
for i in tqdm(range(len(combinations))): 
    ...
    outdir = dir+'/pe/PE_ML2C_{}_{}_{}/'.format(combinations[i][0], combinations[i][1], combinations[i][2])
    
    directory = '/home/nishkal.rao/git_overlap/src/output/pe_population_3G/injections/'
    ...
    ini_file = '/home/nishkal.rao/git_overlap/src/scripts/pe_population_3G/config_ml2c.ini'
    call("cp %s %s"%(ini_file, outdir), shell=1)
    os.chdir(outdir)

    with open('config_ml2c.ini', 'r') as base_file:
    ...
    with open('config_ml2c.ini', 'w') as ini_file: 
        ini_file.write(ini_content)

    call('bilby_pipe config_ml2c.ini --submit', shell=1)