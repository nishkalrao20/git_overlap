...
remote_base_path="/home/anuj.mishra/git_repos/git_overlap/pe_runs/pe_files/ml_rec_pe_res_data" #"~/git_overlap/src/output/pe_population_3G/pe"

# Local destination base directory
local_base_path="git_overlap/src/output/pe_population_3G/final_result"
...
for mc in "${mchirpratio[@]}"; do
    for snr in "${snrratio[@]}"; do
        for dtc in "${delta_tc[@]}"; do
            folder_name="PE_ML_${mc}_${snr}_${dtc}"
            file_name="PE_ML_${i}_data0_1126259462-4702857_analysis_H1L1V1_merge_result.hdf5"
            echo "Copying files for $folder_name..."
            
            # Construct the full remote path
            remote_path="${remote_base_path}/${file_name}" #${folder_name}/outdir/final_result/*
