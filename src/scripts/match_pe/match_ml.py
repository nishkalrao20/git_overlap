...
def log_mismatch_calc(h, mchirp, eta, Ml_z, y, injection_parameters, det):
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter):
    ...
    fun_mismatch = lambda x: log_mismatch_calc(h, x[0], x[1], x[2], x[3], injection_parameters, det)
...
plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Redshifted Lens Mass $M_{\ell}^z$, and Impact Parameter $y$; with Match: %s'%(np.round(match_f_H1, 3)))
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_pe/MATCH_ML.png')
plt.close()
...
print('Recovered (Lensed) - PAIRS: Chirp Mass: %s, Sym. Ratio: %s, Mass1: %s, Mass2: %s; Redshifted Lens Mass: %s, Impact Parameter: %s; Match: %s'%(np.round(mchirpf_H1, 3), np.round(etaf_H1, 3), np.round(mass_1f_H1, 3), np.round(mass_2f_H1, 3), np.round(Ml_zf_H1, 3), np.round(yf_H1, 3), np.round(match_f_H1, 3)))
...
ht_lens = inject_wf_lens(injection_parameters_0, injection_parameters_0['m_lens'], injection_parameters_0['y_lens'])
...
ax.legend()
ax.grid(True)
plt.title('Recovered Lensed Waveforms: Match: %s'%(np.round(match_f_H1, 3)))
plt.savefig('git_overlap/src/output/match_pe/WAVEFORMS_ML.png')
plt.show()
plt.close()
