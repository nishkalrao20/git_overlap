...
def inject_pairs_pycbc(injection_parameters_a, injection_parameters_b):
    ...
    ht_a, ht_b = inject_singles_pycbc(injection_parameters_a), inject_singles_pycbc(injection_parameters_c)
...
ht = inject_pairs_pycbc(injection_parameters_a, injection_parameters_b)

write_frame("git_overlap/src/output/match_pe/injections/PAIRS_H1.gwf", "H1:PyCBC_Injection", ht['H1'])
write_frame("git_overlap/src/output/match_pe/injections/PAIRS_L1.gwf", "L1:PyCBC_Injection", ht['L1'])
write_frame("git_overlap/src/output/match_pe/injections/PAIRS_V1.gwf", "V1:PyCBC_Injection", ht['V1'])
...
def inject_pairs(injection_parameters_a, injection_parameters_b):
    ...
    ht_a, ht_b = inject_wf(injection_parameters_a), inject_wf(injection_parameters_c)
...
def log_mismatch_calc(h, mchirp, eta, injection_parameters, det):
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter):
    ...
    fun_mismatch = lambda x: log_mismatch_calc(h, x[0], x[1], injection_parameters, det)
    
    # Minimizing the mismatch over all the initial set of points 

    log_mismatch, params = [], []
    for i in range(int(N_iter)):
        res = sp.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}, tol=1e-3) 
...
plt.suptitle('Match: Convergence of Chirp Mass $\\mathcal{M}$ and Sym Mass Ratio $\\eta$; with Match: %s'%(np.round(match_f_H1, 3)))
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_pe/MATCH.png')
plt.close()
...
print('Recovered - PAIRS: Chirp Mass: %s, Sym. Ratio: %s, Mass1: %s, Mass2: %s; Match: %s'%(np.round(mchirpf_H1, 3), np.round(etaf_H1, 3), np.round(mass_1f_H1, 3), np.round(mass_2f_H1, 3), np.round(match_f_H1, 3)))

ht = inject_pairs(injection_parameters_a, injection_parameters_b)

injection_parameters_recovered = injection_parameters_a.copy()
for key in injection_parameters_recovered:
    injection_parameters_recovered[key] = (injection_parameters_a[key]+injection_parameters_b[key])/2

injection_parameters_recovered['mass_1'], injection_parameters_recovered['mass_2'] = mass_1f_H1, mass_2f_H1
injection_parameters_recovered['geocent_time'] = injection_parameters_a['geocent_time']

ht_recovered = inject_wf(injection_parameters_recovered)

fig, ax = plt.subplots(figsize=(16, 9))
ax.plot(ht['H1'].sample_times, ht['H1'], 'k--', linewidth=1, label='$\\rm{Injected}$')
ax.plot(ht_recovered['H1'].sample_times, ht_recovered['H1'], 'm-', linewidth=1, label='$\\rm{Recovered}$')
ax.set_xlabel('Time $[s]$')
ax.set_ylabel('Strain $h$')
ax.set_xlim(injection_parameters_a['geocent_time']-0.6, injection_parameters_a['geocent_time']+0.2)
ax.legend()
ax.grid(True)
plt.title('Recovered Waveforms: Match: %s'%(np.round(match_f_H1, 3)))
plt.savefig('git_overlap/src/output/match_pe/WAVEFORMS.png')
plt.close()
