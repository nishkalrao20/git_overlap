...
def mismatch(h, mchirp, eta, luminosity_distance, injection_parameters, det, psd):
    ...
    if np.isnan(mass_1) or np.isnan(mass_2) or mass_1 < 2 or mass_2 < 2 or mass_1/mass_2 < 1/18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800 or luminosity_distance < 50 or luminosity_distance > 3000:
        ...
        injection_parameters['redshift'] = bilby.gw.conversion.luminosity_distance_to_redshift(luminosity_distance)
...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_snr/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
N_sampl = 25
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, snr_b, luminosity_distance_b = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_snr/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Luminosity distance $d_L$, with SNR$_A=$ %s, SNR$_B=$ %s'%(np.abs(waveform_metadata_a['H1']['optimal_SNR']), np.abs(waveform_metadata_b[k]['H1']['optimal_SNR'])))
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_snr/outputs/PAIRS(3D)SNR %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
mc12_c = ax[0].scatter(snr_b, mchirpf_snr, c = match_f_snr, cmap='plasma')
...
ax[0].set_xlabel('SNR$_B$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs SNR$_B$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(snr_b, etaf_snr, c = match_f_snr, cmap='plasma')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('SNR$_B$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs SNR$_B$')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
dL12_c = ax[2].scatter(snr_b, luminosity_distancef_snr, c = match_f_snr, cmap='plasma')
ax[2].axhline(y=luminosity_distance_a, color='r', linestyle='--', label='${d_L}_A$')
ax[2].set_ylabel('$d_L$')
ax[2].set_xlabel('SNR$_B$')
ax[2].set_title('Recovered Luminosity Distance $d_L$ vs SNR$_B$')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_snr/PAIRS(3D)SNR.png')
plt.close()