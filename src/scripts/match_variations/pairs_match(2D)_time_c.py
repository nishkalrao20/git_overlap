# 2 Dimensional (Chirp mass, Symmetric mass ratio) Nelder Mead maximization of Match on PAIRS waveforms, by varying coalescence time of SINGLES_B relative to SINGLES_A
...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_time_c/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
N_sampl = 11
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, delta_tc = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)

for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_time_c/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
print(waveform_metadata_a, waveform_metadata_b[0])
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$ and Sym Mass Ratio $\\eta$ with $\\Delta t_c=$ %s'%delta_tc[k])
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_time_c/outputs/PAIRS(2D)TIME %s.png'%(k+1))
    plt.close()
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_time_c/PAIRS(2D)TIME.png')
plt.close()