...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_time_mchirp/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
N_sampl = 20
waveform_metadata_b = []
mass_1_b, mass_2_b, q_b, mchirp_b, eta_b, delta_tc = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_time_mchirp/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$ and Sym Mass Ratio $\\eta$ with Chirp masses $\\mathcal{M}_A=$ %s, $\\mathcal{M}_B=$ %s, $\\Delta t_c=$ %s'%(mchirp_a, mchirp_b[k], delta_tc[k]))
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_time_mchirp/outputs/PAIRS(2D)TIME_MCHIRP %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,2, figsize=(12, 6))
mc12_c = ax[0].scatter(range(len(mchirp_b)), mchirpf_snr, c = match_f_snr, cmap='plasma')
ax[0].set_xticks(range(len(mchirp_b)), ['({}, {})'.format(a, b) for a, b in zip(np.round(mchirp_b, 2), np.round(delta_tc,2))], rotation=45, ha='right')
ax[0].axhline(y=mchirp_a, color='r', linestyle='--', label='$\\mathcal{M}_A$')
ax[0].set_ylabel('$\mathcal{M}$')
ax[0].set_xlabel('$(\\mathcal{M}_B, \ \\Delta t_c)$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs Injected $(\\mathcal{M}_B, \ \\Delta t_c)$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(range(len(mchirp_b)), etaf_snr, c = match_f_snr, cmap='plasma')
ax[1].set_xticks(range(len(mchirp_b)), ['({}, {})'.format(a, b) for a, b in zip(np.round(mchirp_b, 2), np.round(delta_tc,2))], rotation=45, ha='right')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$(\\mathcal{M}_B, \ \\Delta t_c)$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs Injected $(\\mathcal{M}_B, \ \\Delta t_c)$')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_time_mchirp/PAIRS(2D)TIME_MCHIRP.png')
plt.close()