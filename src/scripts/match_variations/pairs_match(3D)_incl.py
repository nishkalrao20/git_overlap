# 3 Dimensional (Chirp mass, Symmetric mass ratio, Inclination) Nelder Mead maximization of Match on PAIRS waveforms, by varying Inclination of SINGLES_B
...
def mismatch(h, mchirp, eta, incl, injection_parameters, det, psd):
    """Calculating the match/overlap defined as through a noise-weighted inner product in some frequency band maximized over a time and pase of coalescence"""

    mass_1, mass_2 = mconv(mchirp, eta)

    if np.isnan(mass_1) or np.isnan(mass_2) or mass_1 < 2 or mass_2 < 2 or mass_1/mass_2 < 1/18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800 or incl < 0 or incl > np.pi:
        log_mismatch = 1e6
        ...
        injection_parameters['incl'] = incl
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter, k):
    ...
    incl_a, incl_b = injection_parameters_a['incl'], injection_parameters_b['incl']
    ...
    incl = (incl_a*np.abs(waveform_metadata_a[det]['optimal_SNR'])+incl_b*np.abs(waveform_metadata_b[k][det]['optimal_SNR']))/(np.abs(waveform_metadata_a[det]['optimal_SNR'])+np.abs(waveform_metadata_b[k][det]['optimal_SNR']))
    ...
    incl_0 = scipy.stats.uniform.rvs(size = int(1e4))*10*incl

    # Selecting the physically alllowed solutions

    idx = (mchirp_0>1) & (mchirp_0<200) & (eta_0>0.02) & (eta_0<0.25) & (incl_0>0) & (incl_0<np.pi)
    ...
    incl_arr = np.append(incl, np.random.choice(incl_0[idx], int(N_iter)-1)) 
    ...
    for i in range(int(N_iter)):
        res = scipy.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i], incl_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}) 
...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_incl/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
mass_1_a, mass_2_a, incl_a = waveform_metadata_a['H1']['parameters']['mass_1'], waveform_metadata_a['H1']['parameters']['mass_2'], waveform_metadata_a['H1']['parameters']['incl']
...
N_sampl = 25
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, incl_b = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_incl/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
    ...
    incl_b[k] = waveform_metadata_b[k]['H1']['parameters']['incl']

# Evaluating the log(mismatch) values for the PAIRS

mchirpf_ar, etaf_ar, inclf_ar, match_f_ar = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    ...
    incl_H1 = [[params_H1[i][j][2] for j in range(len(params_H1[i]))] for i in range(int(N_iter))]
    ...
    incl_arr_H1 = [incl_H1[i][len(incl_H1[i])-1] for i in range(N_iter)]
    ...
    match_f_ar[k] = match_f_H1

    mchirpf_H1, etaf_H1, inclf_H1 = mchirp_arr_H1[min_idx], eta_arr_H1[min_idx], incl_arr_H1[min_idx]
    mass_1f_H1, mass_2f_H1 = mconv(mchirpf_H1, etaf_H1)
    mchirpf_ar[k], etaf_ar[k], inclf_ar[k] = mchirpf_H1, etaf_H1, inclf_H1
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Inclination $\\iota$, with injected inclinations $\\iota_A=$ %s, $\\iota_B=$ %s'%(waveform_metadata_a['H1']['parameters']['incl'], waveform_metadata_b[k]['H1']['parameters']['incl']))
    ...
    for i in range(N_iter):
        ...
        ax3.plot(incl_H1[i], c=cmap(normalize(match_H1[i])))
    ...
    ax3.set_title('Inclination $\\iota$') 
    ax3.axhline(y=incl_a, color='r', linestyle='--', label='$\\iota_A$')
    ax3.axhline(y=incl_b[k], color='r', linestyle='--', label='$\\iota_B$')
    ax4.set_title('Mass Ratio $\\eta$ $-$ Chirp Mass $\\mathcal{M}$ $-$ Inclination $\\iota$')
    ax4.set_xlabel('$\mathcal{M}$')
    ax4.set_ylabel('$\eta$')
    ax4.set_zlabel('$\\iota$')
    sc = ax4.scatter(mchirp_arr_H1, eta_arr_H1, incl_arr_H1, c=match_H1, cmap='plasma')
    cb = plt.colorbar(sc, ax=ax4)
    cb.set_label('$Match$')
    ax4.scatter(mchirpf_H1, etaf_H1, inclf_H1, facecolors='None', edgecolors='black')
    ax4.scatter(mchirp_a, eta_a, incl_a, facecolors='None', edgecolors='black')
    ax4.scatter(mchirp_b[k], eta_b[k], incl_b[k], facecolors='None', edgecolors='black')
    ax4.text(mchirp_a, eta_a, incl_a, 'A', verticalalignment='top', horizontalalignment='left')
    ax4.text(mchirp_b[k], eta_b[k], incl_b[k], 'B', verticalalignment='top', horizontalalignment='left')
    ax4.text(mchirpf_H1, etaf_H1, inclf_H1, 'F', verticalalignment='bottom', horizontalalignment='right')
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_incl/outputs/PAIRS(3D)INCL %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
mc12_c = ax[0].scatter(incl_b, mchirpf_ar, c = match_f_ar, cmap='plasma')
...
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs Inclination $\\iota_B$ (SINGLES_B)')
ax[0].set_xlabel('$\\iota_B$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(incl_b, etaf_ar, c = match_f_ar, cmap='plasma')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$\\iota_B$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs Inclination $\\iota_B$ (SINGLES_B)')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
dL12_c = ax[2].scatter(incl_b, inclf_ar, c = match_f_ar, cmap='plasma')
ax[2].axhline(y=incl_a, color='r', linestyle='--', label='${\\iota}_A$')
ax[2].set_ylabel('$\\iota$')
ax[2].set_xlabel('$\\iota_B$')
ax[2].set_title('Recovered Inclination $\\iota$ vs Inclination $\\iota_B$ (SINGLES_B)')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_incl/PAIRS(3D)INCL.png')
plt.close()