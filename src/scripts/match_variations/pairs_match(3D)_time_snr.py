...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_time_snr/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_time_snr/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
...
for k in range(N_sampl):
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Luminosity distance $d_L$, with $\\Delta t_c=$ %s, $\\mathrm{SNR}_A=$ %s, $\\mathrm{SNR}_B=$ %s'%(delta_tc[k], snr_a, snr_b[k]))
    ...
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_time_snr/outputs/PAIRS(3D)TIME_SNR %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
mc12_c = ax[0].scatter(range(len(snr_b)), mchirpf_snr, c = match_f_snr, cmap='plasma')
ax[0].set_xticks(range(len(snr_b)), ['({}, {})'.format(a, b) for a, b in zip(np.round(np.divide(snr_b, snr_a),2), np.round(delta_tc,2))], rotation=45, ha='right')
...
ax[0].set_xlabel('$(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs $(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(range(len(snr_b)), etaf_snr, c = match_f_snr, cmap='plasma')
ax[1].set_xticks(range(len(snr_b)), ['({}, {})'.format(a, b) for a, b in zip(np.round(np.divide(snr_b, snr_a),2), np.round(delta_tc,2))], rotation=45, ha='right')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs $(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
dL12_c = ax[2].scatter(range(len(snr_b)), luminosity_distancef_snr, c = match_f_snr, cmap='plasma')
ax[2].set_xticks(range(len(snr_b)), ['({}, {})'.format(a, b) for a, b in zip(np.round(np.divide(snr_b, snr_a),2), np.round(delta_tc,2))], rotation=45, ha='right')
ax[2].axhline(y=luminosity_distance_a, color='r', linestyle='--', label='${d_L}_A$')
ax[2].set_ylabel('$d_L$')
ax[2].set_xlabel('$(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
ax[2].set_title('Recovered Luminosity Distance $d_L$ vs $(\\mathrm{SNR}_B/\\mathrm{SNR}_A, \ \\Delta t_c)$')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_time_snr/PAIRS(3D)TIME_SNR.png')
plt.close()