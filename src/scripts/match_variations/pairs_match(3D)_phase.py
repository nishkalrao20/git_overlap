# 3 Dimensional (Chirp mass, Symmetric mass ratio, Phase) Nelder Mead maximization of Match on PAIRS waveforms, by varying Phase of SINGLES_B
...
def mismatch(h, mchirp, eta, phase, injection_parameters, det, psd):
    ...
    if np.isnan(mass_1) or np.isnan(mass_2) or mass_1 < 2 or mass_2 < 2 or mass_1/mass_2 < 1/18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800 or phase < 0 or phase > 2*np.pi:
        log_mismatch = 1e6
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter, k):
    ...
    phase_a, phase_b = injection_parameters_a['phase'], injection_parameters_b['phase']
    ...
    phase = (phase_a*np.abs(waveform_metadata_a[det]['optimal_SNR'])+phase_b*np.abs(waveform_metadata_b[k][det]['optimal_SNR']))/(np.abs(waveform_metadata_a[det]['optimal_SNR'])+np.abs(waveform_metadata_b[k][det]['optimal_SNR']))
    ...
    phase_0 = scipy.stats.uniform.rvs(size = int(1e4))*10*phase

    # Selecting the physically alllowed solutions

    idx = (mchirp_0>1) & (mchirp_0<200) & (eta_0>0.02) & (eta_0<0.25) & (phase_0>0) & (phase_0<2*np.pi)
    ...
    phase_arr = np.append(phase, np.random.choice(phase_0[idx], int(N_iter)-1)) 
    ...
    for i in range(int(N_iter)):
        res = scipy.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i], phase_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}) 
...
waveform_metadata_a = pickle.load(open('git_overlap/src/output/match_variations/match_phase/injections/GW Waveform A Meta Data.pkl', 'rb'))   # Importing Waveform Meta Data
mass_1_a, mass_2_a, phase_a = waveform_metadata_a['H1']['parameters']['mass_1'], waveform_metadata_a['H1']['parameters']['mass_2'], waveform_metadata_a['H1']['parameters']['phase']
...
N_sampl = 25
waveform_metadata_b = []
mass_1_b, mass_2_b, mchirp_b, eta_b, phase_b = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    waveform_metadata_b.append(pickle.load(open('git_overlap/src/output/match_variations/match_phase/injections/GW Waveform B Meta Data %s.pkl'%(k+1), 'rb')))   # Importing Waveform Meta Data
    ...
    phase_b[k] = waveform_metadata_b[k]['H1']['parameters']['phase']

# Evaluating the log(mismatch) values for the PAIRS

mchirpf_ar, etaf_ar, phasef_ar, match_f_ar = np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl), np.zeros(N_sampl)
for k in range(N_sampl):
    ...
    phase_H1 = [[params_H1[i][j][2] for j in range(len(params_H1[i]))] for i in range(int(N_iter))]
    ...
    phase_arr_H1 = [phase_H1[i][len(phase_H1[i])-1] for i in range(N_iter)]
    ...
    mchirpf_H1, etaf_H1, phasef_H1 = mchirp_arr_H1[min_idx], eta_arr_H1[min_idx], phase_arr_H1[min_idx]
    mass_1f_H1, mass_2f_H1 = mconv(mchirpf_H1, etaf_H1)
    mchirpf_ar[k], etaf_ar[k], phasef_ar[k] = mchirpf_H1, etaf_H1, phasef_H1
    ...
    plt.suptitle('Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$ and Phase $\\phi$, with Phase difference $\\phi_B-\\phi_A=$ %s'%(waveform_metadata_b[k]['H1']['parameters']['phase']))
    ...
    for i in range(N_iter):
        ...
        ax3.plot(phase_H1[i], c=cmap(normalize(match_H1[i])))
    ...
    ax3.axhline(y=phase_a, color='r', linestyle='--', label='$\\phi_A$')
    ax3.axhline(y=phase_b[k], color='r', linestyle='--', label='$\\phi_B$')
    ax4.set_title('Mass Ratio $\\eta$ $-$ Chirp Mass $\\mathcal{M}$ $-$ Phase $\\phi$')
    ax4.set_xlabel('$\mathcal{M}$')
    ax4.set_ylabel('$\eta$')
    ax4.set_zlabel('$\\phi$')
    sc = ax4.scatter(mchirp_arr_H1, eta_arr_H1, phase_arr_H1, c=match_H1, cmap='plasma')
    cb = plt.colorbar(sc, ax=ax4)
    cb.set_label('$Match$')
    ax4.scatter(mchirpf_H1, etaf_H1, phasef_H1, facecolors='None', edgecolors='black')
    ax4.scatter(mchirp_a, eta_a, phase_a, facecolors='None', edgecolors='black')
    ax4.scatter(mchirp_b[k], eta_b[k], phase_b[k], facecolors='None', edgecolors='black')
    ax4.text(mchirp_a, eta_a, phase_a, 'A', verticalalignment='top', horizontalalignment='left')
    ax4.text(mchirp_b[k], eta_b[k], phase_b[k], 'B', verticalalignment='top', horizontalalignment='left')
    ax4.text(mchirpf_H1, etaf_H1, phasef_H1, 'F', verticalalignment='bottom', horizontalalignment='right')
    fig.tight_layout()
    plt.savefig('git_overlap/src/output/match_variations/match_phase/outputs/PAIRS(3D)PHASE %s.png'%(k+1))
    plt.close()

fig, ax = plt.subplots(1,3, figsize=(18, 6))
mc12_c = ax[0].scatter(phase_b, mchirpf_ar, c = match_f_ar, cmap='plasma')
...
ax[0].set_xlabel('$\\phi_B-\\phi_A$')
ax[0].set_title('Recovered Chirp Mass $\\mathcal{M}$ vs Phase Difference $\\phi_B-\\phi_A$')
clb1 = plt.colorbar(mc12_c, ax=ax[0])
clb1.ax.set_title('$Match$')
et12_c = ax[1].scatter(phase_b, etaf_ar, c = match_f_ar, cmap='plasma')
ax[1].axhline(y=eta_a, color='r', linestyle='--', label='$\\eta_A$')
ax[1].axhline(y=eta_b[0], color='r', linestyle='--', label='$\\eta_B$')
ax[1].set_ylabel('$\\eta$')
ax[1].set_xlabel('$\\phi_B-\\phi_A$')
ax[1].set_title('Recovered Symmetric Mass Ratio $\\eta$ vs Phase Difference $\\phi_B-\\phi_A$')
clb2 = plt.colorbar(et12_c, ax=ax[1])
clb2.ax.set_title('$Match$')
dL12_c = ax[2].scatter(phase_b, phasef_ar, c = match_f_ar, cmap='plasma')
ax[2].axhline(y=phase_a, color='r', linestyle='--', label='${\\phi}_A$')
ax[2].set_ylabel('$\\phi$')
ax[2].set_xlabel('$\\phi_B-\\phi_A$')
ax[2].set_title('Recovered Phase $\\phi$ vs Phase Difference $\\phi_B-\\phi_A$')
clb3 = plt.colorbar(dL12_c, ax=ax[2])
clb3.ax.set_title('$Match$')
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_variations/match_phase/PAIRS(3D)PHASE.png')
plt.close()