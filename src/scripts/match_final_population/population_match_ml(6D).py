...
# 5 Dimensional Microlensed (Chirp mass, Mass ratio, Spin1z, Spin2z, Mlz, y) Nelder Mead maximization of Match on PAIRS waveforms
...
for k, idx in enumerate(indices):
    ...
    ht = gen.pairs_ml_fd(injection_parameters_a = waveform_metadata_a[k]['H1']['parameters'], injection_parameters_b = waveform_metadata_b[k]['H1']['parameters'], **kwargs)
    FF_res = ff.compute_fitting_factor(ht['H1'], wf_model='ML_6D', apx="IMRPhenomXPHM" , f_lower=f_lower, f_high=f_high, psd=None,
    ...
    np.savetxt('/home/nishkal.rao/git_overlap/src/output/match_final_population/outputs_ml/PAIRS(6D) %s.csv'%(idx), np.column_stack((FF_res[0], recovered_mchirp, recovered_q, recovered_Sz1, recovered_Sz2, recovered_Mlz, recovered_y)), delimiter=',')