...
# 4 Dimensional (Chirp mass, Mass ratio, Spin1z, Spin2z) Nelder Mead maximization of Match on PAIRS waveforms
...
keys = ['mass_1_source', 'masZ0EVQHs_ratio', 'a_1', 'a_2', 'redshift', 'cos_tilt_1', 'cos_tilt_2', 'phi_12', 'phi_jl', 'cos_theta_jn', 'ra', 'dec', 'psi', 'phase', 'incl', 'cos_theta_zn', 'mass_1', 'mass_2', 'luminosity_distance', 'tilt_1', 'tilt_2', 'theta_jn', 'theta_zn', 'geocent_time', 'snr_det']
...
for k, idx in enumerate(indices):
    ...
    ht = gen.pairs_td(injection_parameters_a = waveform_metadata_a[k]['H1']['parameters'], injection_parameters_b = waveform_metadata_b[k]['H1']['parameters'], **kwargs)
    FF_res = ff.compute_fitting_factor(ht['H1'], wf_model='4D', apx="IMRPhenomXPHM" , f_lower=f_lower, f_high=f_high, psd=None,
    ...
    np.savetxt('/home/nishkal.rao/git_overlap/src/output/match_final_population/outputs/PAIRS(4D) %s.csv'%(idx), np.column_stack((FF_res[0], recovered_mchirp, recovered_q, recovered_Sz1, recovered_Sz2)), delimiter=',')
