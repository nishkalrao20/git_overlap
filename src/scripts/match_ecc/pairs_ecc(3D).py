...
plt.suptitle('PAIRSvsECCENTRIC: Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Eccentricity $e$')
...
fig.tight_layout()
plt.savefig('/home/nishkal.rao/git_overlap/src/output/match_ecc/pairs_ecc/PAIRS(3D).png')
plt.close()
...
print('Recovered - PAIRS: Chirp Mass: %s, Sym. Ratio: %s, Mass1: %s, Mass2: %s; Eccentricity: %s; Match: %s'%(np.round(mchirpf_H1, 3), np.round(etaf_H1, 3), np.round(mass_1f_H1, 3), np.round(mass_2f_H1, 3), np.round(ef_H1, 3), np.round(match_f_H1, 3)))
...
ax.legend()
ax.grid(True)
plt.savefig('/home/nishkal.rao/git_overlap/src/output/match_ecc/pairs_ecc/WAVEFORMS.png')
plt.show()
plt.close()
