...
def mismatch(h, mchirp, eta, injection_parameters, det, psd):
    ...
    if np.isnan(mass_1) or np.isnan(mass_2) or mass_1 < 2. or mass_2 < 2 or mass_1/mass_2 < 1./18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800:
        ...
        h_templ = inject_wf_ecc(injection_parameters, 0)[det] 
...
def match(injection_parameters_a, injection_parameters_b, det, N_iter):
    ...
    for i in range(int(N_iter)):
        res = sp.optimize.minimize(fun_mismatch, (mchirp_arr[i], eta_arr[i]), method='Nelder-Mead', options={'adaptive':True, 'return_all': True}, tol=1e-4) 
...
# Importing the waveform metadata and setting the initial parameters                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
...
injection_parameters_b = {'mass_1': 40, 
                        'mass_2': 20, 
                        'luminosity_distance': 750, 
                        'a_1': 0, 
                        'a_2': 0, 
                        'tilt_1': 0, 
                        'tilt_2': 0, 
                        ...
                        'geocent_time': 1126259462.4702857+0.2}
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_pe/PAIRS(2D).png')
plt.close()
...
print('Recovered - PAIRS: Chirp Mass: %s, Sym. Ratio: %s, Mass1: %s, Mass2: %s; Match: %s'%(np.round(mchirpf_H1, 3), np.round(mass_1f_H1, 3), np.round(mass_2f_H1, 3), np.round(match_f_H1, 3)))
...
ht_recovered = inject_wf_ecc(injection_parameters_recovered, 0)
...
plt.savefig('git_overlap/src/output/match_pe/WAVEFORMS(NCC).png')
plt.close()
