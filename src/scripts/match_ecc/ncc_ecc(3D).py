...
def match(injection_parameters_a, det, N_iter):
    ...
    h, psd = inject_wf_ecc(injection_parameters_a, 0)[det], read_psd(det)
...
plt.suptitle('NON-ECCENTRICvsECCENTRIC: Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Eccentricity $e$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_ecc/ncc_ecc/PAIRS(3D).png')
plt.close()

# Printing the required results

print('Injected - NON-ECCENTRIC: \n %s'%(injection_parameters_a))
...
ht = inject_wf_ecc(injection_parameters_a, 0)
...
ax.legend()
ax.grid(True)
plt.savefig('git_overlap/src/output/match_ecc/ncc_ecc/WAVEFORMS.png')
plt.show()
plt.close()
