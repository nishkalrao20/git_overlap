...
def mismatch(h, mchirp, eta, e, injection_parameters, det, psd):
    ...
    if np.isnan(mass_1) or np.isnan(mass_2) or mass_1 < 2. or mass_2 < 2 or mass_1/mass_2 < 1./18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800 or e<0. or e>0.9:
        log_mismatch = 1e6
...
def match(injection_parameters_a, det, N_iter):
    ...
    mchirp = np.power(mass_1_a*mass_2_a, 3/5)/np.power(mass_1_a+mass_2_a, 1/5)
    eta = (mass_1_a*mass_2_a)/np.power(mass_1_a+mass_2_a, 2)
    
    # Generating the PAIRS waveforms and Interpolating the PSD
    
    h, psd = inject_wf_ecc(injection_parameters_a, 0.3)[det], read_psd(det)
    ...
    fun_mismatch = lambda x: mismatch(h, x[0], x[1], x[2], injection_parameters_a, det, psd)
...
injection_parameters_a = {'mass_1': 30, 
                        'mass_2': 30, 
...
log_mismatch_H1, match_H1, params_H1 = match(injection_parameters_a, 'H1', int(N_iter))
...
plt.suptitle('ECCENTRICvsECCENTRIC: Convergence of Chirp Mass $\\mathcal{M}$, Sym Mass Ratio $\\eta$, Eccentricity $e$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match_ecc/ecc_ecc/PAIRS(3D).png')
plt.close()

# Printing the required results
injection_parameters_a['eccentricity'] = 0.3
print('Injected - ECCENTRIC: \n %s'%(injection_parameters_a))
...
ht = inject_wf_ecc(injection_parameters_a, 0.3)

injection_parameters_0 = injection_parameters_a.copy()
for key in injection_parameters_0:
    injection_parameters_0[key] = injection_parameters_a[key]
...
ax.set_xlim(injection_parameters_a['geocent_time']-1, injection_parameters_a['geocent_time']+0.2)
ax.legend()
ax.grid(True)
plt.savefig('git_overlap/src/output/match_ecc/ecc_ecc/WAVEFORMS.png')
plt.show()
plt.close()
