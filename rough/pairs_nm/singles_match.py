# 4 Dimensional (Chirp mass, Symmetric mass ratio, Dimensionless Spin magnitudes) Nelder Mead maximization of Match on SINGLES waveforms
...
def mismatch(h, mchirp, eta, a_1, a_2, injection_parameters, det, psd):
    ...
    if np.isnan(mass_1) or np.isnan(mass_2) or np.isnan(a_1) or np.isnan(a_2) or mass_1 < 2. or mass_2 < 2 or mass_1/mass_2 < 1./18 or mass_1/mass_2 > 18 or mass_1+mass_2 > 800 or a_1 < 0.01 or a_1 > 0.99 or a_2 < 0.01 or a_2 > 0.99:
        ...
        log_mismatch = np.log10(1-match)   # Evaluating the log(mismatch)
...
def match(injection_parameters, det, N_iter):
    ...
    # Generating the waveforms and Interpolating the PSD
    
    h, psd = inject_wf(injection_parameters)[det], read_psd(det)

    # Distributing the initial array of points as a truncated normal continuous rv around the injected values
...
log_mismatch_a_H1, match_a_H1, mchirp_arr_a_H1, eta_arr_a_H1, a_1_arr_a_H1, a_2_arr_a_H1 = match(waveform_metadata_a['H1']['parameters'], 'H1', N_iter)
min_idx_a, max_idx_a = np.argmin(log_mismatch_a_H1), np.argmax(match_a_H1)
log_mismatch_af_H1, match_af_H1 = log_mismatch_a_H1[min_idx_a], match_a_H1[max_idx_a]
mchirpf_a_H1, etaf_a_H1, a_1f_a_H1, a_2f_a_H1 = mchirp_arr_a_H1[min_idx_a], eta_arr_a_H1[min_idx_a], a_1_arr_a_H1[min_idx_a], a_2_arr_a_H1[min_idx_a]
mass_1f_a_H1, mass_2f_a_H1 = mconv(mchirpf_a_H1, etaf_a_H1)

print(log_mismatch_a_H1,  mchirp_arr_a_H1, eta_arr_a_H1, a_1_arr_a_H1, a_2_arr_a_H1, log_mismatch_af_H1, match_af_H1, mchirpf_a_H1, etaf_a_H1, a_1f_a_H1, a_2f_a_H1)

log_mismatch_b_H1, match_b_H1, mchirp_arr_b_H1, eta_arr_b_H1, a_1_arr_b_H1, a_2_arr_b_H1 = match(waveform_metadata_b['H1']['parameters'], 'H1', N_iter)
min_idx_b, max_idx_b = np.argmin(log_mismatch_b_H1), np.argmax(match_b_H1)
log_mismatch_bf_H1, match_bf_H1 = log_mismatch_b_H1[min_idx_b], match_b_H1[max_idx_b]
mchirpf_b_H1, etaf_b_H1, a_1f_b_H1, a_2f_b_H1 = mchirp_arr_b_H1[min_idx_b], eta_arr_b_H1[min_idx_b], a_1_arr_b_H1[min_idx_b], a_2_arr_b_H1[min_idx_b]
...
plt.suptitle('\\textbf{SINGLES A: MATCH}')

# Mchirp - Eta
mc12_c = ax[0].scatter(mchirp_arr_a_H1, eta_arr_a_H1, c = match_a_H1, cmap = 'plasma')
...
# a_1 - a_2
s12_c = ax[1].scatter(a_1_arr_a_H1, a_2_arr_a_H1, c = match_a_H1, cmap = 'plasma')
ax[1].scatter(a_1_a, a_2_a, facecolors = 'None', edgecolors = 'black')
ax[1].scatter(a_1f_a_H1, a_2f_a_H1, facecolors = 'None', edgecolors = 'black')
ax[1].text(a_1_a, a_2_a,'A', verticalalignment ='top', horizontalalignment = 'left')
ax[1].text(a_1f_a_H1, a_2f_a_H1,'F', verticalalignment ='bottom', horizontalalignment = 'right')
ax[1].set_xlabel('$a_1$')
ax[1].set_ylabel('$a_2$')
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match/SINGLES A.png')
plt.close()

fig, ax = plt.subplots(1,2, figsize = (13, 6))
plt.suptitle('\\textbf{SINGLES B: MATCH}')

# Mchirp - Eta
mc12_c = ax[0].scatter(mchirp_arr_b_H1, eta_arr_b_H1, c = match_b_H1, cmap = 'plasma')
...
# a_1 - a_2
s12_c = ax[1].scatter(a_1_arr_b_H1, a_2_arr_b_H1, c = match_b_H1, cmap = 'plasma')
ax[1].scatter(a_1_b, a_2_b, facecolors = 'None', edgecolors = 'black')
ax[1].scatter(a_1f_b_H1, a_2f_b_H1, facecolors = 'None', edgecolors = 'black')
ax[1].text(a_1_b, a_2_b,'B', verticalalignment ='top', horizontalalignment = 'left')
ax[1].text(a_1f_b_H1, a_2f_b_H1,'F', verticalalignment ='bottom', horizontalalignment = 'right')
ax[1].set_xlabel('$a_1$')
ax[1].set_ylabel('$a_2$')
ax[1].set_title('Dimensionless Spin magnitudes: $a_1-a_2$')
clb2 = plt.colorbar(s12_c, ax=ax[1])
...
fig.tight_layout()
plt.savefig('git_overlap/src/output/match/SINGLES B.png')
plt.close()
...
print('Recovered - SINGLES A: Chirp Mass: %s, Sym. Ratio: %s, a_1: %s, a_2: %s, Mass1: %s, Mass2: %s; Match: %s'%(np.round(mchirpf_a_H1, 3), np.round(etaf_a_H1, 3), np.round(a_1f_a_H1, 3), np.round(a_2f_a_H1, 3), np.round(mass_1f_a_H1, 3), np.round(mass_2f_a_H1, 3), np.round(match_af_H1, 3)))
...
print('Recovered - SINGLES B: Chirp Mass: %s, Sym. Ratio: %s, a_1: %s, a_2: %s, Mass1: %s, Mass2: %s; Match: %s'%(np.round(mchirpf_b_H1, 3), np.round(etaf_b_H1, 3), np.round(a_1f_b_H1, 3), np.round(a_2f_b_H1, 3), np.round(mass_1f_b_H1, 3), np.round(mass_2f_b_H1, 3), np.round(match_bf_H1, 3)))