...
\end{equation} 
%-------------
where $x$ and $y$ are the image- and source-plane coordinates in units of the lens scale $\xi_0$ (typically the Einstein radius), and $T(x,y)$ is the dimensionless time-delay function. The dimensionless frequency is
%-------------
\begin{equation}
...
When $w \gg 1$ for the frequencies of interest, only stationary points of $T(x, y)$ contribute significantly to Eq.~\eqref{eq:amplification}, and the amplification factor for image $j$ reduces to~\cite{Ezquiaga:2020gdt, Dai:2017huk},
%-------------
\begin{equation}
...
This study uses Type-II strong-lensing and point-mass microlensing models as inference templates for overlapping GW signals. Both models can produce characteristic amplitude and phase modulations; a $-\pi/2$ phase shift for Type-II images in the geometric-optics limit, and oscillatory patterns in the wave-optics regime for microlensing. These patterns may be similar to overlapping signatures, creating a potential for degeneracy. These models have been employed in GW lensing studies~\cite{Meena:2019ate, Basak:2021ten, Mishra:2021xzz, Vijaykumar:2022dlp, Janquart:2023mvf, LIGOScientific:2023bwz, Mishra:2023ddt, Mishra:2023vzo, Wright:2024mco} and are therefore well-suited for the PE and FF analyses presented in this study.
However, it is worth noting that model-independent approaches for detecting lensed signals also exist~\cite{Liu:2023ikc, Chakraborty:2025maj}, although they are not utilized in this study.
...
\end{equation}
%-------------
Here, $S_n(f)$ is the power spectral density (PSD) of the detector, and $f_{\min}$ and $f_{\max}$ are the analysis frequency bounds\footnote{Typically, $f_{\min}$ corresponds to the lowest frequency where the detector noise can be approximated as stationary, while $f_{\max}$ denotes the Nyquist frequency.}. We use the target PSDs for the O4 observing run of the Advanced LIGO and Virgo detectors\footnote{For the LIGO detectors, we use the PSD provided \href{https://dcc.ligo.org/public/0165/T2000012/002/aligo_O4high.txt}{here}, and for Virgo, we use the PSD available \href{https://dcc.ligo.org/public/0165/T2000012/002/avirgo_O4high_NEW.txt}{here}.}~\citep{Abbott:2020search}.
...
\begin{table*}
    ...
    \midrule
    $y$: Impact Parameter & -- & \textsc{PowerLaw}$_{\alpha=2}$($0.01,5$)\\
...
Optimization over intrinsic parameters is performed using Differential Evolution (DE), which ensures robustness across parameter space, unlike the Nelder-Mead (NM) method~\cite{Lagarias:1998iqa, Jaranowski:2005hz, Ajith:2007kx, Ajith:2007xh,  CalderonBustillo:2015lrt,  Basak:2021ten, Mishra:2023ddt, Ajith:2024inj}, which is biased by the initial data. Extrinsic parameters are excluded as they only affect the waveform through an overall amplitude scaling and phase shift. %We further verify the convergence of these runs rigorously across the parameter space, and compare it with the NM simplex method as elaborated in Appendix~\ref{sec:dfconvergence}. 
...
\begin{figure*}[htb!]
     ...
     \caption{Inference of overlapping signals using microlensed templates for an isolated point-mass lens. \emph{Left:} Inferred logarithmic redshifted lens mass $\log_{10} M_{\rm L}^z/{\rm M}_{\odot}$. \emph{Middle:} Inferred dimensionless impact parameter $y$. \emph{Right:} Bayes factor comparison between microlensed and unlensed hypotheses, with circular markers indicating $\log_{10}\mathscr{B}^{\rm L}_{\rm U} \geq 1$.}
...
N.R. wishes to thank P. Relton and J. Janquart for valuable discussions and feedback. N.R. further acknowledges support from KVPY for funding. 
...
The analysis codes for the PE and FF analyses of overlapping GW signals used in this work are publicly made available at the repository on Github:~\href {https://github.com/nishkalrao20/git_overlap}{\textcolor{blue!50}{\faGithubSquare}}.

% \appendix

% \section{Differential Evolution Convergence}
% \label{sec:dfconvergence}

% \begin{figure}
%      \centering
%      \includegraphics[keepaspectratio, width=0.485\textwidth]{plots/ConvergencePlot.pdf}
%      \caption{Comparison of optimization trajectories on an objective function, whose contour levels are shown. We have (i) two independent NM simplex runs with different initial points, plotted as connected triangular frames, and (ii) DE populations (scatter) and the DE best-solution trajectory (red line). The final converged points (in black) are distinct for the different initial seed points in NM, which is not observed in DE. DE provides the most accurate estimate of the function's minima, as compared to analytical methods.}
%      \label{fig:convergence}
% \end{figure}

% We adopted the DE algorithm, introduced by~\cite{Storn:1997uea}, to address optimisation challenges posed by the result dependency in the choice of initial seed points in deterministic methods used in earlier studies, including NM optimisation technique~\cite{Nelder:1965zz, Gao:2012guu}. DE is a robust, derivative-free algorithm that initialises a population of candidate solutions across the parameter space. New solutions are iteratively generated by combining and mutating existing candidates, enabling exploration of the solution space. Its stochastic nature makes it particularly effective for complex optimisation problems, though it typically requires more function evaluations than gradient-based techniques. Trial candidates are generated by randomly selecting parameters and sequentially filling them using a modulo operation, updating the best solution vector continuously within each iteration to ensure convergence.

% We monitored the evolution of the best-fit solutions as they traversed the parameter space, minimising mismatch values between the generated template and the overlapping waveform. Multiple-dimensional DE minimisations were conducted across key parameters, including the base and the lensing parameters. DE consistently achieved better convergence results than the NM method, demonstrating its effectiveness for our analyses. We further evaluated both the methods against several runs with known injections and inferences of a single signal (unlensed and lensed), with both the runs admitting $100\%$ convergence over the runs. Fig.~\ref{fig:convergence} demonstrates the superiority of DE in convergence and its seed point independence. Although both DE and NM have achieved convergence, DE is more aligned with the analytical result.
